    ...
    : _hasPowerDifficultyData(false)
{
    ...
    // SpellPowerEntry
    PowerCost = sObjectMgr->GetSpellPower(Id);
...
SpellPowerCost SpellInfo::CalcPowerCost(Unit const* caster, SpellSchoolMask schoolMask) const
{
    SpellPowerCost cost{};

    auto power = sObjectMgr->GetSpellPower(Id);
    if (!power)
        return cost;
